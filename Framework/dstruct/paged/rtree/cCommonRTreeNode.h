...
#ifndef __cCommonRTreeNode_h__
#define __cCommonRTreeNode_h__
...
class cCommonRTreeNode : public cTreeNode<TItem>
{
private:
...
public:
	...
	cCommonRTreeNode(cTreeNodeHeader* header, const char* mem);
	cCommonRTreeNode(const cCommonRTreeNode<TItem>* origNode, const char* mem);
...
template<class TItem> bool cCommonRTreeNode<TItem>::DIFFERENT_CARDINALITY = false;

/**
 * This contructor is used in the case when the node is created on the pool, this pool creates a memory
 * and the node uses it.
 */template<class TItem> 
cCommonRTreeNode<TItem>::cCommonRTreeNode(const cCommonRTreeNode<TItem>* origNode, const char* mem): parent(origNode, mem)
{
}

 template<class TItem> 
cCommonRTreeNode<TItem>::cCommonRTreeNode(cTreeNodeHeader* header, const char* mem): parent(header, mem)
{
}

template<class TItem> 
void cCommonRTreeNode<TItem>::Clear() 
{ 
	parent::mItemCount = 0; 
...
template<class TItem> 
void cCommonRTreeNode<TItem>::UpdateItem(unsigned int order, const char *item, const cSpaceDescriptor* pSd)
{
	TItem::Copy(parent::GetKeyPtr(order), item, pSd); //fk GetItem->GetKey
}

template<class TItem> 
void cCommonRTreeNode<TItem>::UpdateMbr(unsigned int order, const char *TKey_item, const cSpaceDescriptor* pSd)
{
...
template<class TItem> 
bool cCommonRTreeNode<TItem>::TestConsistence(const unsigned int maximal_value)
{
 ...
 **/
template<class TItem> 
void cCommonRTreeNode<TItem>::CreateNewRootNode(cTreeHeader *mHeader, char* tMbr1, tNodeIndex nIndex1, char* tMbr2, tNodeIndex nIndex2)
{
...
template<class TItem> 
void cCommonRTreeNode<TItem>::Print(const cSpaceDescriptor* pSd, sItemBuffers* buffers) const
{
	tNodeIndex index;
	if (parent::IsLeaf())
	{
