...
bool cStream::Seek(llong pos, DWORD origin)
{
	if (m_handle != (HANDLE)NULL)
	{
		LARGE_INTEGER li;
...
cInputStream::cInputStream(const char* AFName, DWORD mode): cStream()
{
	Open(AFName, mode);
}
...
#else
bool cInputStream::Open(const char* AFName, DWORD mode)
#endif
{
...
cOutputStream::cOutputStream(const char* AFName, DWORD mode): cStream()
{
	Open(AFName, mode);
}

#ifdef UNICODE
bool cOutputStream::Open(WCHAR* AFName, const DWORD mode, const DWORD flag)
#else
bool cOutputStream::Open(const char* AFName, const DWORD mode, const DWORD flag)
#endif
{
	m_handle = CreateFile(AFName, 
               GENERIC_WRITE,
...
cIOStream::cIOStream(const char* AFName, DWORD mode): cStream()
{
	Open(AFName, mode);
}

#ifdef UNICODE
bool cIOStream::Open(WCHAR* AFName, DWORD mode)
#else
bool cIOStream::Open(const char* AFName, DWORD mode)
#endif
{
	return Open(AFName, mode, FILE_FLAG_NO_BUFFERING | FILE_FLAG_WRITE_THROUGH);
	//return Open(AFName, mode, FILE_FLAG_RANDOM_ACCESS);
}

#ifdef UNICODE
bool cIOStream::Open(WCHAR* AFName, DWORD mode, const DWORD flags)
#else
bool cIOStream::Open(const char* AFName, DWORD mode, const DWORD flags)
#endif
{
	m_handle = CreateFile(AFName, 
