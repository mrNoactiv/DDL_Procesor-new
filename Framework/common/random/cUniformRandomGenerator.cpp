...
/**
 * Call it in the case of the empty constructor.
 */
void cUniformRandomGenerator::Init(const bool Randomize)
{
	idum2 = 123456789;
	iy = 0;
	int j, k;
 ...
 */
unsigned int cUniformRandomGenerator::GetNextUInt(unsigned int maxValue)
{
	double rn = GetNext();
	if (rn < 0)
		{ rn = -rn; }
